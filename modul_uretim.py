...
def go_home(): 
    # Uygulamanın ana sayfasına döner
...
def goster():
    ...
    # --- 0. ANA MENÜ (BUTON GERİ GELDİ) ---
    if st.session_state.uretim_page == 'menu':
        # İşte o silinen buton:
        if st.button("⬅️ ANA MENÜYE DÖN"): 
            go_home()
            st.rerun()
            
        st.subheader("🏭 Üretim Hazırlık Modülü (v18.0)")
        st.markdown("---")
    ...
    # --- 1. YÜKLEME (DOĞRU EŞLEŞME İÇİN VERİ HAZIRLAMA) ---
        ...
        if uploaded_file:
            try:
                ...
                # Birebir eşleşme için Mamül Adını aşağıya doğru doldur (ffill)
                ...
                # Sadece gerçek malzemeleri filtrele
    ...
    # --- 2. HAZIRLIK (ÜÇLÜ KİLİTLEME: MAMÜL + KOD + AD) ---
        ...
        if not df_db.empty:
            ...
            if sel_is != "Seçiniz...":
                ...
                # Personelin önüne Ürün + Malzeme Kodu + Malzeme Adı üçlüsünü getiriyoruz (Hatasız seçim)
    ...
    # --- 3. RAPOR (BİREBİR EŞLEŞME RAPORU VE EXCEL) ---
