...
def goster(): # app.py bu ismi bekliyor, ismini 'goster' olarak sabitledik
    ...
    # --- 1. YÜKLEME (Öğe Etiketlerini Yinele ve Sekme Esnekliği Eklendi) ---
        ...
        if uploaded_file:
            try:
                ...
                # Dinamik Sekme Seçimi
                target_sheet = None
                if "HAZIRLIK" in sheet_names: target_sheet = "HAZIRLIK"
                elif "Sheet4" in sheet_names: target_sheet = "Sheet4"
                ...
                st.dataframe(df_final_save, use_container_width=True)
                ...
                if st.button("VERİTABANINA ŞİMDİ KAYDET", type="primary"):
                    ...
                    st.success(f"✅ {target_sheet} sekmesindeki etiketler yinelenerek kaydedildi!")
                    st.cache_data.clear(); st.rerun()
            except Exception as e: st.error(f"Hata: {e}")

    # --- 2. OPERASYON (HAYA Mantığı ve Tek Tablo Düzeni) ---
        ...
        if not df_emirler.empty:
            ...
            if s_list:
                ...
                with st.container(border=True):
                    ...
                    input_isim = p1.selectbox("📝 Malzeme:", ["Seçiniz..."] + bekleyenler)
                    ...
                    if input_isim != "Seçiniz...":
                        ...
                        p2.text_input("📊 İhtiyaç:", value=f"{int(row['İhtiyaç Miktarı'])}", disabled=True)
                        ...
                        adrs_list = temp_stok[stok_adr_col].unique().tolist() if not temp_stok.empty else ["STOK YOK"]
                        input_adr = p3.selectbox("📍 Adres:", adrs_list)
                        input_mik = p4.number_input("🔢 Miktar:", min_value=0.0, step=1.0)

                        if st.button("⚡ KAYDET", use_container_width=True, type="primary"):
                            # HAYA ise stok artar, değilse azalır
                            ...
                            # İş Emri Güncelle
                            kalan = input_mik
                            emir_idx = st.session_state.local_emirler[(st.session_state.local_emirler['İş Emri'].isin(s_list)) & (st.session_state.local_emirler['Stok Kodu'] == sel_kod)].index
                            for idx in emir_idx:
                                if kalan <= 0: break
                                bosluk = st.session_state.local_emirler.at[idx, 'İhtiyaç Miktarı'] - st.session_state.local_emirler.at[idx, 'Hazırlanan Adet']
                                alinacak = min(kalan, max(bosluk, 0))
    ...
    # --- 3. RAPOR (Excel Butonu ve Ürün Filtresi) ---
