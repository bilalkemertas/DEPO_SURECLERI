...
def run_blok_kesim(conn):
    ...
    try:
        ...
        # SÜTUN İSİMLERİNDEKİ GİZLİ BOŞLUKLARI TEMİZLE
    ...
    with st.container(border=True):
        ...
        if uploaded_file:
            try:
                ...
                # --- KRİTİK VERİ TEMİZLİĞİ (EŞLEŞME GARANTİSİ) ---
                # Excel tarafındaki kodları temizle (Nokta, virgül, boşluk ve .0 kısmını atar)
                df_main['Malzeme Kodu'] = df_main['Malzeme Kodu'].astype(str).str.replace(r'[\.,\s]', '', regex=True).str.replace(r'\.0$', '', regex=True).str.strip()
                
                # Sheets tarafındaki kodları temizle
            ...
            if not match.empty:
                ...
                if pd.notna(item['BRN Kod']):
                    with st.container(border=True):
                        ...
                        st.caption(f"BRN: {item['BRN Kod']} | Kat: {item['Kategori']}")
