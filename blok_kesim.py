...
def run_blok_kesim(conn):
    ...
    if 'main_data' in st.session_state:
        ...
        # Sütun isimlerini güncelledik (Sipariş No, Blok Ölçüsü, Plaka Kodu, Plaka Adı, Adet)
        tanim_col = "Plaka Adı"
        miktar_col = "Adet"
        kod_col = "Plaka Kodu"
        ...
        for idx, row in df.iterrows():
            ...
            if plaka_kodu and eslesme_matrix is not None and not eslesme_matrix.empty:
                ...
                matris_blok_kod_col = eslesme_matrix.columns[1]
                matris_blok_adi_col = eslesme_matrix.columns[2]
        ...
        if barkod:
            ...
            if not match.empty:
                ...
                def get_is_suitable(row):
                    # Matris Kontrolü
                    pk = str(row.get(kod_col, '')).split('.')[0].strip()
                    if eslesme_matrix is not None and not eslesme_matrix.empty:
                        matris_kod_col = eslesme_matrix.columns[0]
                        matris_blok_kod_col = eslesme_matrix.columns[1]
                        m_match = eslesme_matrix[eslesme_matrix[matris_kod_col] == pk]
                        if not m_match.empty and blok_kod in m_match[matris_blok_kod_col].astype(str).str.strip().tolist():
                ...
                uygunlar = df[df.apply(get_is_suitable, axis=1)].copy()
