...
# --- 6. STOK İŞLEMLERİ ---
    ...
    with t1:
        is_t = st.selectbox("İşlem:", ["GİRİŞ", "ÇIKIŞ"], key="st_t")
        sec = st.selectbox("Hızlı Seçim:", ["+ MANUEL GİRİŞ"] + katalog_list, key="st_s")
        k_i = sec.split(" | ")[0] if sec != "+ MANUEL GİRİŞ" else ""
        i_i = sec.split(" | ")[1] if sec != "+ MANUEL GİRİŞ" else ""
        kod = st.text_input("Stok Kodu:", value=k_i, key="st_k").strip().upper()
        isim = st.text_input("Stok Adı:", value=i_i if i_i else find_name_by_code(kod), key="st_i").strip().upper()
        adr = st.text_input("Adres:", value="GENEL", key="st_a").strip().upper()
        qty = st.number_input("Miktar:", min_value=0.1, key="st_q")
        if st.button("KAYDET", use_container_width=True, type="primary", key="st_b"):
            if is_t == "ÇIKIŞ":
    ...
    with t2:
        ...
        t_kod = st.text_input("Ürün Kodu:", value=t_sec.split(" | ")[0] if t_sec != "+ MANUEL GİRİŞ" else "", key="tr_k").strip().upper()
        t_qty = st.number_input("Miktar:", min_value=0.1, key="tr_q")
        if st.button("TRANSFERİ ONAYLA", use_container_width=True, type="primary", key="tr_b"):
            stok_df = get_internal_data("Stok")
            ...
            if mev >= t_qty:
                ...
                st.success("Transfer İşlendi!")
            else: st.error(f"Stok Yok! Mevcut: {mev}")

    with t3:
        search = st.text_input("🔍 Stok Sorgula:", key="sq_in").strip().upper()
...
# --- 7. ÜRETİM HAZIRLIK ---
    ...
    if st.button("⬅️ ANA MENÜ", key="n_u"): go_home(); st.rerun()
            ...
            try:
                ...
                if kc and ac and mc:
                    ...
                    if st.button(f"'{eno}' Kaydet", key="u_s_b"):
                        ...
                        st.success("Eklendi!"); st.cache_data.clear()
