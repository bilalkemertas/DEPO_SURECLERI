...
# ──────────────────────────────────────────────────────────
# YENİ: Aranabilir/önerili adres seçici. Depo_Adresler.xlsx üzerinden
# öneri sunar; listede olmayan bir adres gerekirse "+ MANUEL" seçilip
# elle yazılabilir. Kaynak/Hedef adres alanlarındaki 3 tekrarı önlemek
# için tek fonksiyona çıkarıldı.
# ──────────────────────────────────────────────────────────
def _adres_secici(label, key_prefix):
    adres_listesi = veritabani.get_adres_listesi()
    secenekler = ["+ MANUEL"] + adres_listesi
    sec = st.selectbox(label, secenekler, key=f"{key_prefix}_secim")
    if sec != "+ MANUEL":
        return sec
    manuel = st.text_input(f"{label} (elle):", key=f"{key_prefix}_manuel")
    return manuel.upper().strip() if manuel.strip() else "UNKNOWN"
...
def goster():
    ...
    with st.container(border=True):
        ...
        # DÜZELTME: Serbest metin yerine Depo_Adresler.xlsx üzerinden
        # öneri/arama yapılabilen açılır liste (_adres_secici).
        src_adr, dst_adr = None, None
        a1, a2 = st.columns(2)

        if move_type == "GİRİŞ":
            with a1:
                dst_adr = _adres_secici("📍 Hedef Adres:", "dst")

        elif move_type == "ÇIKIŞ":
            with a1:
                src_adr = _adres_secici("📍 Kaynak Adres:", "src")
