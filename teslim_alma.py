...
def handle_barcode():
    ...
    if found.empty:
        st.toast(f"❌ Barkod SAS listesinde yok: {code}", icon="🚫"); return
    ...
    # --- MAL KABUL GİRİŞ ---
        ...
        if st.session_state.mk_gecici_liste:
            if st.button("🚀 STOĞA AKTARIMI TAMAMLA", type="primary", use_container_width=True):
                ...
                st.session_state.mk_gecici_liste = {}; st.success("✅ Depoya başarıyla işlendi!"); st.rerun()
